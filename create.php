...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($post_clean['phone'])) {
        ...
        } else if (is_invalid_phone($post_clean['phone'])) {
    ...
    //var_dump($errors); die();
    //Si le tableau contient au moins une erreur
    if (count($errors) > 0) {
        //Sauvegardons les msg d'erreurs en session.
        $_SESSION['create_form_errors'] = $errors;

        //Sauvegardons les données provenants du formulaire en session.
        $_SESSION['create_form_old_values'] = $post_clean;

        //Effectuons une redirections vers la page de laquelle provient les informations
        //Puis, arrêtons l'éxecution du script
        return header("Location: " . $_SERVER['HTTP_REFERER']);
    }
    ...
    //Effectuons la requête d'insertion des données dans la table "contacts"
    create_contact([
    ...
    //Générons un msg à afficher à l'utilisateur pour lui expliquer que son nouveau contact a bien été ajouté à la liste
    $_SESSION['success'] = "Le contact a été ajouter à la liste avec succès.";
            ...
            <div class="col-md-8 col-lg-7 mx-auto p-4 shadow bg-white">
                ...
                <form method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="comment" id="create_form_comment" class="form-control" rows="4" value="<?= isset($_SESSION['create_form_old_values']['comment']) ? $_SESSION['create_form_old_values']['comment'] : '';
